...
const STORAGE_KEY = 'mik-bank-state-ultimate';
...
function readInitialState(): BankingState {
  try {
    ...
    return saved ? (JSON.parse(saved) as BankingState) : initialBankingState;
  } catch {
...
export function useBankingState() {
  ...
  function resetDemoData() {
    ...
    setState(initialBankingState);
    notify('info', 'Демо-данные восстановлены');
  }
  ...
  function openAccount(payload: { name: string; currency: CurrencyCode; type: Account['type'] }) {
    const account: Account = {
      id: crypto.randomUUID(),
      name: payload.name || 'Новый счёт',
      number: createAccountNumber(),
      balance: 0,
      currency: payload.currency,
      type: payload.type,
      status: 'active',
      interestRate: payload.type === 'saving' ? 12.5 : undefined,
  ...
  function closeAccount(accountId: string) {
    ...
    setState((current) => ({
      ...
      accounts: current.accounts.map((item) =>
        item.id === accountId ? { ...item, status: 'closed' } : item,
      ),
    }));
  ...
  function transfer(payload: { fromAccountId: string; toAccountNumber: string; amount: number }) {
    ...
    if (!account || payload.amount <= 0 || payload.toAccountNumber.length < 8) {
      notify('error', 'Проверьте данные перевода');
      return;
    }

    if (account.balance < payload.amount) {
      notify('error', 'Недостаточно средств');
      return;
    }

    const transaction = createTransaction({
      title: `Перевод на ${payload.toAccountNumber.slice(-4)}`,
      category: 'Переводы',
    ...
    setState((current) => ({
      ...current,
      accounts: current.accounts.map((item) =>
        item.id === account.id ? { ...item, balance: item.balance - payload.amount } : item,
      ),
      transactions: [transaction, ...current.transactions],
    }));
  ...
  function pay(payload: { accountId: string; title: string; amount: number }) {
    ...
    const transaction = createTransaction({
      title: payload.title || 'Оплата услуг',
      category: 'Платежи',
      amount: -payload.amount,
      currency: account.currency,
      accountId: account.id,
  ...
  function exchange(payload: { fromAccountId: string; toCurrency: CurrencyCode; amount: number }) {
    const account = state.accounts.find((item) => item.id === payload.fromAccountId);
    const rate = state.rates.find((item) => item.code === payload.toCurrency);

    if (!account || payload.amount <= 0 || !rate || account.balance < payload.amount) {
    ...
    const transaction = createTransaction({
      title: `Обмен ${account.currency} → ${payload.toCurrency}`,
      category: 'Валюта',
      amount: -payload.amount,
      currency: account.currency,
      accountId: account.id,
  ...
  function submitAction(action: Action, payload: Record<string, string | number>) {
    ...
    if (action === 'topup') {
      topUp(String(payload.accountId), Number(payload.amount));
    }
  ...
  function sendAssistantMessage(text: string) {
    ...
    let answer =
      'Я могу подсказать по счетам, переводам, валюте, кэшбэку и последним операциям. Позже ответ будет приходить из Assistant Service.';
    ...
    if (lower.includes('расход') || lower.includes('операц')) {
      const last = state.transactions
        ...
        .join('; ');
      answer = `Последние операции: ${last}.`;
    }
    ...
    if (lower.includes('накоп')) {
      answer =
        'Накопительный счёт можно открыть в разделе «Счета». В демо-данных ставка указана 12.5% годовых.';
    }

    if (lower.includes('валют') || lower.includes('курс')) {
      const rates = state.rates.map((rate) => `${rate.code}: ${rate.sell.toFixed(2)} ₽`).join(', ');
      answer = `Текущие демо-курсы продажи: ${rates}.`;
    }

    if (lower.includes('кэшб') || lower.includes('cashback')) {
      const categories = state.cashbackCategories
        .filter((category) => category.selected)
        .map((category) => `${category.title} ${category.percent}%`)
        .join(', ');
      answer = `Выбранные категории кэшбэка: ${categories}.`;
    }
